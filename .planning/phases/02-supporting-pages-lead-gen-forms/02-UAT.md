...
number: 1
name: Contact form delivers a real inquiry (Web3Forms key + inbox)
expected: |
  After replacing TODO_WEB3FORMS_ACCESS_KEY in src/config.ts with a real key from
  web3forms.com (tied to Milan's destination email), submitting the /contact form
  shows the inline Dutch success message AND the inquiry arrives in the inbox.
  Before the key is set, the form must surface the error path — never a fake success.
